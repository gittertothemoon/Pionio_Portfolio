const { chromium } = require('playwright');
const path = require('path');
const fs = require('fs');
const HTML_PATH = path.resolve(__dirname, 'slides.html');
const OUT_DIR = path.resolve(__dirname, '..', 'out');
const ASSET = path.resolve(__dirname, '..', 'assets', 'frame-raw.png');
if (!fs.existsSync(ASSET)) {
  console.error('Missing asset: ' + ASSET);
  console.error('Genera l\'immagine AI col prompt in brief-prompt.md e salvala come frame-raw.png.');
  process.exit(1);
}
(async () => {
  const browser = await chromium.launch();
  const context = await browser.newContext({ viewport: { width: 1080, height: 1350 }, deviceScaleFactor: 2 });
  const page = await context.newPage();
  await page.goto('file://' + HTML_PATH, { waitUntil: 'networkidle' });
  await page.screenshot({ path: path.join(OUT_DIR, 'frame.png'), clip: { x: 0, y: 0, width: 1080, height: 1350 }, omitBackground: false });
  console.log('OK frame.png');
  await browser.close();
})().catch((err) => { console.error(err); process.exit(1); });
